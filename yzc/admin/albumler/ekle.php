...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $albumsira=$conn->albumsirabul();
    $sira=$albumsira['sira']+1;
    ...
    if (empty($error)) {
        ...
        $flag = $conn->albumekle($baslik, $kapak,$tarih,1,$sira);
        if ($flag) {
            $message = "Albüm Başarıyla Eklendi";
            $allowed_extension = array("jpg", "jpeg", "png", "pdf", "doc", "docx");
            $count = 0;
            if (!empty($_FILES['galeri']['name'][0])) {
                if (!file_exists('../../documents/albumler/' . $flag)) {
                    mkdir('../../documents/albumler/' . $flag, 0777, true);
                }
                foreach ($_FILES['galeri']['name'] as $filename) {
                    $temp = '../../documents/albumler/' . $flag . '/';
        ...
        <div>
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h4 class="m-0 font-weight-bold text-black">Albüm Ekle</h4>
                </div>
                <div class="card-body">
